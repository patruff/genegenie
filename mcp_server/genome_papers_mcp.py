#!/usr/bin/env python3
"""
Genome Papers MCP Server

An MCP (Model Context Protocol) server that provides RAG (Retrieval Augmented Generation)
capabilities for genomics research papers using Google's File Search Tool.

Features:
- Upload PDFs (GWAS, SNP studies, genomics papers) to Google's File Search store
...
- Integrated with GeneGenie's SNP extraction and trait analysis
...
# Server instance
server = Server("genome-papers-rag")
...
def get_or_create_store() -> tuple[Any, str]:
    """Get or create the file search store with display name 'genomepapers'.

    This creates a SEPARATE store from longevitypdf for genome-specific papers.
    All genomics PDFs should be uploaded to this store for unified RAG queries.
    """
...
async def handle_list_tools() -> list[mcp_types.Tool]:
    ...
    return [
        mcp_types.Tool(
            name="upload_genome_paper",
            description=(
                "Upload a genomics research paper (PDF) to the RAG system. "
                "Perfect for GWAS studies, SNP papers, trait genetics, longevity research. "
        ...
        mcp_types.Tool(
            name="query_genome_papers",
            description=(
                "Query the genomics papers knowledge base using natural language. "
                "Returns AI-generated answers grounded in the uploaded papers with citations. "
                "Perfect for questions about SNPs, GWAS findings, trait genetics, "
                "intelligence/personality genetics, addiction genetics, etc. "
                "Examples: 'What SNPs are associated with intelligence?', "
                "'Which genes affect alcohol dependence?', "
                "'What are the APOE variants and their effects?'"
            ),
            inputSchema={
                "type": "object",
                "properties": {
                    "query": {
                        "type": "string",
                        "description": "Natural language question about genomics research"
                    },
                    "model": {
                        "type": "string",
        ...
        mcp_types.Tool(
            name="list_indexed_genome_papers",
            description=(
                "List all genomics papers currently indexed in the RAG system. "
                "Shows file names, sizes, and statistics for all uploaded documents."
            ),
            inputSchema={
                "type": "object",
                "properties": {}
        ...
        mcp_types.Tool(
            name="delete_genome_paper",
            description=(
                "Delete a specific paper from the genome RAG system by its file ID."
            ),
            inputSchema={
                "type": "object",
                "properties": {
                    "file_id": {
                        "type": "string",
                        "description": "The file ID to delete (from list_indexed_genome_papers)"
                    }
                },
                "required": ["file_id"]
            }
...
async def handle_call_tool(
    ...
    try:
        if name == "upload_genome_paper":
            return await upload_genome_paper(arguments or {})
        elif name == "query_genome_papers":
            return await query_genome_papers(arguments or {})
        elif name == "list_indexed_genome_papers":
            return await list_indexed_genome_papers(arguments or {})
        elif name == "get_genome_store_info":
            return await get_genome_store_info(arguments or {})
        elif name == "delete_genome_paper":
            return await delete_genome_paper(arguments or {})
        else:
...
async def upload_genome_paper(arguments: dict[str, Any]) -> list[mcp_types.TextContent]:
    ...
    try:
        ...
        logger.info(f"Uploading {file_path.name} to genome papers store...")
        ...
        return [mcp_types.TextContent(
            ...
            text=(
                ...
                f"The paper is now searchable in the genome papers RAG system.\n"
                f"Store: {store_name}\n\n"
                f"You can now query it using the 'query_genome_papers' tool!\n"
                f"Try asking about SNPs, genes, or traits mentioned in the paper."
            )
        )]
...
async def query_genome_papers(arguments: dict[str, Any]) -> list[mcp_types.TextContent]:
    ...
    try:
        ...
        logger.info(f"Querying genome papers with: {query}")
...
async def list_indexed_genome_papers(arguments: dict[str, Any]) -> list[mcp_types.TextContent]:
    """List all indexed papers in the store."""
    try:
        client = get_client()
        _, store_name = get_or_create_store()

        logger.info("Listing indexed genome papers...")
        ...
        if not doc_list:
            return [mcp_types.TextContent(
                type="text",
                text="No genome papers indexed yet. Use 'upload_genome_paper' to add papers."
            )]
        ...
        result += f"**Store**: `{store_name}` (genomepapers)"
...
async def get_genome_store_info(arguments: dict[str, Any]) -> list[mcp_types.TextContent]:
    ...
    try:
        ...
        info = f"## Genome Papers File Search Store Information\n\n"
        info += f"**Store Name:** `{store_name}`\n"
        info += f"**Display Name:** `{STORE_DISPLAY_NAME}`\n"
        info += f"**Config Path:** `{config_path}`\n\n"
        info += "### Note\n"
        info += "This is a SEPARATE store from longevitypdf for genome-specific papers.\n\n"
        info += "### Pricing\n"
        info += "- **Storage:** FREE\n"
...
async def delete_genome_paper(arguments: dict[str, Any]) -> list[mcp_types.TextContent]:
    ...
    if not file_id:
        return [mcp_types.TextContent(
            type="text",
            text="Error: file_id is required. Use 'list_indexed_genome_papers' to get file IDs."
        )]
...
async def main():
    """Run the MCP server."""
    logger.info("Starting Genome Papers MCP Server...")
